...
void rwa2::Mouse::move_forward(){ //moves the mouse one step forward
    API::moveForward();
}

void rwa2::Mouse::turn_left(){ //turns the mouse to its left (90 CCW)
    API::turnLeft();
    m_direction = (m_direction+3)%4; //changing the direction to WEST of mouse
}

void rwa2::Mouse::turn_right(){ // turns the mouse to its right (90 CW)
    API::turnRight();
    m_direction = (m_direction+1)%4; //changing the direction to EAST of mouse
}

bool rwa2::Mouse::search_maze(){

    while (depth_first_search(m_x,m_y)){ // If there is a path this loop will be executed

        bool next_node_flag = false; // this flag is set true if the mouse can't be moved to the next node.
        size_t size_ns = m_node_stack.size(); // size_t variable to store node stack size
        ...
        for(size_t i=0; i<size_ns; i++){
            ...
            if(i != 0){
                API::setColor(m_node_stack.top().at(0),m_node_stack.top().at(1),'g'); // colors the tiles of computed path from present mouse node to goal node in green
            }
            else{ 
                // sets the goal node text to G1, G2, G3 (or) G4 based on the goal node indices
        ...
        // std::cerr << "Mouse movement starts\n";
        while(true){ // Loops until a new unknown wall is encountered by the mouse during its transversal
            bool flag = false;
            ...
            m_next_node = m_final_path.top(); // Assigning the next node to the new top node of path stack
        ...
        if(next_node_flag){ // condition is true when mouse can't move to the next node
...
bool rwa2::Mouse::depth_first_search(int l_x, int l_y){
    API::clearAllColor(); // clear the tiles color before generating a new path
    if(l_x != m_goal_node.at(0) || l_y != m_goal_node.at(1)){
        if(m_node_stack.empty()){ // if stack is empty, push the current node to the stack
    ...
    if(!m_node_stack.empty()){
        ...
        return depth_first_search(m_curr_node.at(0),m_curr_node.at(1));
    }
    else{
        return false;
    }
